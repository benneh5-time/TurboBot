...
def post_thread(session, game_title, security_token, setup):
    ...
    if setup == "closedrandom10er":
        ...
        game_flavor = f"This is a closed and random 10er. Roles have been randomly selected from a pool of roles Turby has access to that is ever growing. <br><br>This cannot rand as mountainous.<br><br>The village rands between 1 and 2 PRs. If the village rands 1 PR, the wolves rand between 0 and 1 PRs. If the village rands 2 PRs, the wolves rand between 1 and 2 PRs. There is no weight assigned to any power roles--any variation of these setups is possible and balance is not guaranteed.<br><br> There are at most 2 PRs for the village and at most 2 for the wolves. <br><br>These are the roles possible for the village: <br><br>{town_flavor}<br><br>These are the roles possible for wolves:<br><br>{mafia_flavor}<br><br><br>Suffix Legend:<br>de - Disabled in Endgame<br>c - Compulsive<br>m - Macho<br>st - Self-Targetable<br>gf - Godfather"
    else:
